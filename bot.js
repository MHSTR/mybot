...
if(message.content == '<@488814558501142539>') {
...
message.react("📩")
    ...
    if(rokz.content.startsWith(prefix + "تقديم")) {
      ...
      await rokz.channel.send("وش الفرق بين let و var ؟").then(e => {
       ...
       .then(col => {
         time = col.first().content;
          col.first().delete();
            ...
            e.edit(`كيف نستخدم ال await في كود معين

مثال :
await message.channel.send("supports test")
...
if(message.guild.id !==  '431397251445817345') return;
...
client.on('message', function(message) {
    if (message.channel.type === "dm") {
        ...
        client.channels.get("497125740312002580").send({ embed: stewart });
    }
});
client.on('message' , message => {
  var prefix = "-";
